...
# 13 "MCAL_Layer/Timer0/../std_libraries.h"
...
# 31 "MCAL_Layer/Timer0/../std_libraries.h"
...
# 13 "MCAL_Layer/Timer0/../compiler.h"
...
# 13 "MCAL_Layer/Timer0/../compiler.h" 2
...
# 15 "MCAL_Layer/Timer0/../GPIO/hal_gpio.h"
# 1 "MCAL_Layer/Timer0/../GPIO/../device_config.h" 1
# 15 "MCAL_Layer/Timer0/../GPIO/hal_gpio.h" 2

# 1 "MCAL_Layer/Timer0/../GPIO/hal_gpio_cfg.h" 1
# 16 "MCAL_Layer/Timer0/../GPIO/hal_gpio.h" 2
# 50 "MCAL_Layer/Timer0/../GPIO/hal_gpio.h"
typedef enum{
    GPIO_LOW=0,
    GPIO_HIGH
}logic_t;
...
static __attribute__((inline)) void timer0_prescaler_cfg(const timer0_t *timer);
static __attribute__((inline)) void timer0_mode_select(const timer0_t *timer);
static __attribute__((inline)) void timer0_register_size_cfg(const timer0_t *timer);
...
# 54 "MCAL_Layer/Timer0/hal_timer0.c"
        (INTCONbits.GIE = 1);
        (INTCONbits.PEIE = 1);
...
static __attribute__((inline)) void timer0_prescaler_cfg(const timer0_t *timer){
    if(timer->prescaler_enable == 1){
        (T0CONbits.PSA = 0);
        T0CONbits.T0PS = timer->prescaler_value;
    }
    else if(timer->prescaler_enable == 0){
        (T0CONbits.PSA = 1);
    }
}

static __attribute__((inline)) void timer0_mode_select(const timer0_t *timer){
    if(timer->operation_mode == 1){
        (T0CONbits.T0CS = 0);
    }
    else if(timer->operation_mode == 0){
        (T0CONbits.T0CS = 1);
        if(timer->counter_edge == 1){
            (T0CONbits.T0SE = 0);
        }
        else if(timer->counter_edge == 0){
            (T0CONbits.T0SE = 1);
        }
    }
}
...
static __attribute__((inline)) void timer0_register_size_cfg(const timer0_t *timer){
    if(timer->register_size == 1){
        (T0CONbits.T08BIT = 1);
    }
    else if(timer->register_size == 0){
        (T0CONbits.T08BIT = 0);
    }
}
